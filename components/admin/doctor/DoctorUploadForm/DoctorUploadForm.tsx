    ...
    <div className="max-w-2xl mx-auto p-8 bg-white shadow-xl rounded-2xl border border-gray-100">
      ...
      <form action={clientAction} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Specialty (Optional)</label>
            <input name="specialty" type="text" className="w-full text-black border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-teal-500/20 outline-none" placeholder="Oral Surgery" />
          </div>

          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Experience (Optional)</label>
            <input name="experience" type="text" className="w-full text-black border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-teal-500/20 outline-none" placeholder="10 Years" />
          </div>

          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">University (Optional)</label>
            <input name="university" type="text" className="w-full text-black border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-teal-500/20 outline-none" placeholder="Dhaka Dental College" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <label className={`flex flex-col items-center justify-center w-full text-black h-40 border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer bg-slate-50 hover:bg-teal-50 transition-all ${preview ? 'hidden' : ''}`}>
              ...
              <input id="imageInput" name="image" type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
            </label>
